                ...
                for switch in switches:
                    if switch == 's':
                        ...
                        if self.filename != '':
                            ...
                            extra = ''
                        self.filename = '%s-%s%s.txt' % (
                        os.path.splitext(os.path.basename(sys.argv[0]))[0], extra, self.FormatTime())
                    else:
                        return False
                return True
        return False
...
def ProcessPostPacketDataSub(data, oOutput, oCrypto, dCallbacksSummary, options):
    ...
    if callback in [0, 25]:
        ...
        oOutput.Line(callbackdata.decode("GBK"))
        # decode("unicode_escape")  # windows 中文#
        # decode("GBK")  # windows 中文
...
def AnalyzeCapture(filename, options):
    ...
    for packet in capture:
        ...
        if hasattr(packet.http, 'request') and packet.http.has_field(
...
https://DidierStevens.com'''
    ...
    oParser.add_option('-k', '--hmacaeskeys', type=str, default='',
                       help="HMAC and AES keys in hexadecimal separated by :")
